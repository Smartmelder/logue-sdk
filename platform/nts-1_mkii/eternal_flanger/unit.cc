/*
    ETERNAL FLANGER V2 - COMPLETE FIX!
    
    CRITICAL FIXES:
    - Proper feedback state initialization
    - Correct NaN detection
    - Buffer clearing on init
    - Safe feedback limiting (max 0.85)
    - No more runaway oscillation!
*/
...
inline float delay_read(float *buffer, float delay_samples) {
    ...
    // ✅ FIX: Validate read result
    if (!is_finite(result)) result = 0.f;
...
inline void process_eternal_flanger(float in_l, float in_r, float *out_l, float *out_r) {
    ...
    for (uint8_t i = 0; i < s_active_stages; i++) {
        ...
        float base_delay = 1.f + s_depth * 14.f;
        float mod_delay = lfo * s_depth * 10.f;
        
        float delay_ms = base_delay + mod_delay;
        delay_ms = clipminmaxf(0.5f, delay_ms, 20.f);
        
        float delay_samples = delay_ms * 48.f;
        ...
        // ✅ FIX: Safe feedback (max 0.85!)
        if (s_feedback > 0.01f) {
            float fb_amt = clipminmaxf(0.f, s_feedback, 0.85f) * 0.6f;
            
            stage->feedback_state_l = delayed_l + stage->feedback_state_l * fb_amt;
            stage->feedback_state_r = delayed_r + stage->feedback_state_r * fb_amt;
        ...
        // Resonance
        if (s_resonate > 0.01f) {
            float boost = 1.f + s_resonate * si_fabsf(lfo) * 0.5f;
            delayed_l *= boost;
            delayed_r *= boost;
        }
        ...
        // Update LFO
        float rate_hz = 0.05f + s_rate * 7.95f;
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc) {
    ...
    // ✅ FIX: Initialize stages properly!
    for (uint8_t i = 0; i < NUM_STAGES; i++) {
        s_stages[i].lfo_phase = (float)i / (float)NUM_STAGES;
        s_stages[i].crossfade_level = 1.f;
        s_stages[i].feedback_state_l = 0.f;  // ✅ CRITICAL!
        s_stages[i].feedback_state_r = 0.f;  // ✅ CRITICAL!
    }
...
__unit_callback void unit_set_param_value(uint8_t id, int32_t value) {
    ...
    switch (id) {
        case 0: s_direction = (Direction)value; break;
        case 1: s_rate = valf; break;
        case 2: s_depth = valf; break;
        case 3: s_feedback = valf; break;
        case 4: s_mix = valf; break;
        case 5: s_stereo = valf; break;
        case 6: s_tone = valf; break;
        case 7: s_smooth = valf; break;
        case 8:
            s_active_stages = (uint8_t)value;
            s_active_stages = clipminmaxu32(2, s_active_stages, 4);
            break;
        case 9: s_resonate = valf; break;
        default: break;
    }
}
...
__unit_callback int32_t unit_get_param_value(uint8_t id) {
    switch (id) {
        ...
        case 7: return (int32_t)(s_smooth * 1023.f);
