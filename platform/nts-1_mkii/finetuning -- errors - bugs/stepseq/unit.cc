...
static bool s_is_playing;  // true = playing, false = paused
...
inline float svf_process(float input, float cutoff, float resonance, 
                         float *z1, float *z2) {
    // ✅ FIX: Safe cutoff range (100Hz - 12kHz, not 20Hz-20kHz!)
    cutoff = clipminmaxf(0.15f, cutoff, 0.85f);  // Limit input to 15%-85%
    float freq = 100.f + cutoff * 11900.f;      // 100Hz - 12kHz (safe range)
    freq = clipminmaxf(100.f, freq, 12000.f);   // Hard limit frequency
    ...
    // ✅ FIX: Q-factor limiting (max 3.0, not 10.0!)
    float q = 1.f / (0.5f + resonance * 2.5f);  // Max Q = 3.0
    q = clipminmaxf(0.3f, q, 3.0f);             // Hard limit Q
    
    *z2 = *z2 + f * *z1;
    float hp = input - *z2 - q * *z1;
    *z1 = *z1 + f * hp;
    
    // ✅ FIX: Clip filter states to prevent explosion
    *z1 = clipminmaxf(-2.f, *z1, 2.f);
    *z2 = clipminmaxf(-2.f, *z2, 2.f);
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc)
{
    ...
    s_direction_mode = 0;
    s_is_playing = true;  // Start playing by default
...
__unit_callback void unit_reset()
{
    ...
    // ✅ FIX: Reset filter states to prevent clicks
    s_svf_z1_l = s_svf_z2_l = 0.f;
    s_svf_z1_r = s_svf_z2_r = 0.f;
}
...
__unit_callback void unit_render(const float *in, float *out, uint32_t frames)
{
    ...
    for (uint32_t f = 0; f < frames; f++) {
        ...
        // NEW: Check if sequencer is playing
        if (s_is_playing) {
            ...
            // ✅ FIX: Apply filter modulation with safe range
            float filter_cutoff = 0.15f + current_step_data->filter_mod * 0.7f;  // 15%-85%
            filter_cutoff = clipminmaxf(0.15f, filter_cutoff, 0.85f);           // Hard limit!
            float filter_resonance = 0.25f;  // Moderate Q
            ...
            // NEW: PAUSED - Pass through input unchanged
            out[f * 2] = in_l;
            out[f * 2 + 1] = in_r;
        }
...
__unit_callback void unit_set_param_value(uint8_t id, int32_t value)
{
    ...
    switch (id) {
        ...
        case 10:  // NEW: Play/Pause
            s_is_playing = (value != 0);
            break;
...
__unit_callback int32_t unit_get_param_value(uint8_t id)
{
    switch (id) {
        ...
        case 10: return s_is_playing ? 1 : 0;  // NEW
        default: return 0;
    }
}
